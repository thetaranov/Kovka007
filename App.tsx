        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 animate-fade-in-up">
                ...
                <div className="space-y-3">
                    ...
                    <button onClick={handleCopy} className="w-full bg-[#2AABEE] text-white p-4 rounded-xl flex items-center gap-3 justify-center font-bold shadow-lg shadow-blue-200"><Send size={20} /> <span>Скопировать и перейти в Telegram</span></button>
                </div>
            </div>
        </div>
    );
...
export default function App() {
    ...
    useEffect(() => {
        if (window.Telegram?.WebApp) {
            ...
            try { window.Telegram.WebApp.expand(); } catch (e) { console.warn(e); }
        }
    }, []);
    ...
    useEffect(() => {
        ...
        if (config.hasFoundation) materialCost += pillarCount * 4000;
    ...
    const getOrderPayload = () => {
        ...
        return { id: `CFG-${Date.now().toString(36).toUpperCase().slice(-5)}`, type: config.roofType, width: config.width, length: config.length, height: config.height, price: price, color_frame: frameColorObj ? frameColorObj.name : config.frameColor, color_roof: roofColorObj ? roofColorObj.name : config.roofColor };
    };
    ...
    if (appMode === 'calculator' && calculationMode === 'calculated' && calculationResult) {
        return <FinalModel config={config} calculation={calculationResult} onBack={handleBackToEdit} />;
    }
        ...
        <div className="flex flex-col lg:flex-row h-[100dvh] w-screen overflow-hidden bg-slate-100 font-sans touch-none overscroll-none fixed inset-0">
            <div className="absolute top-0 left-0 right-0 z-40 p-4 pointer-events-none flex justify-between lg:justify-start lg:p-6 items-center">
                <div className="bg-white/90 backdrop-blur-md px-6 py-2 rounded-xl shadow-sm border border-slate-200/50 text-center lg:text-left pointer-events-auto">
                    <h1 className="font-bold text-slate-900 leading-tight">Kovka007 <span className="hidden lg:inline text-slate-400">|</span> <span className="text-xs font-normal text-slate-500 uppercase tracking-wider">конструктор</span></h1>
                </div>
                <div className="bg-white/90 backdrop-blur-md p-1 rounded-xl shadow-sm border border-slate-200/50 pointer-events-auto flex gap-1 lg:ml-6">
                    <button onClick={() => setAppMode('visualizer')} className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors ${appMode === 'visualizer' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:bg-slate-100'}`}><Eye size={14}/> 3D Визуализация</button>
                    <button onClick={() => setAppMode('calculator')} className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors ${appMode === 'calculator' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:bg-slate-100'}`}><HardHat size={14}/> Инженерный расчет</button>
                </div>
            </div>
            <div className="relative w-full flex-grow min-h-0 lg:h-full transition-all duration-300">
                ...
                <div className="absolute bottom-6 left-0 right-0 flex justify-center pointer-events-none z-30 px-4">
                    <div className="bg-white/90 backdrop-blur-md px-4 py-2 rounded-xl shadow-lg border border-slate-200 text-slate-800 flex items-center gap-3 text-xs sm:text-sm font-medium whitespace-nowrap overflow-x-auto hide-scrollbar max-w-full">
                        ...
                        <span className="w-px h-3 bg-slate-300 flex-shrink-0"></span><span className="font-bold text-slate-700">{(config.width * config.length).toFixed(1)} м²</span>
                        <span className="w-px h-3 bg-slate-300 flex-shrink-0"></span><span className="text-slate-500">~{price > 0 && config.width > 0 && config.length > 0 ? Math.round(price / (config.width * config.length)).toLocaleString() : 0} ₽/м²</span>
                    </div>
                </div>
            </div>
            <div className="lg:hidden flex flex-col z-30 flex-shrink-0 bg-white shadow-[0_-4px_20px_rgba(0,0,0,0.05)] pb-safe">
                ...
                <div className="px-4 pt-3"><button onClick={() => setIsMobileMenuOpen(true)} className="w-full bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-bold py-3 rounded-xl flex items-center justify-center gap-2 transition-colors active:scale-95"><Settings2 size={18} /><span>Настроить параметры</span></button></div>
                <div className="p-4">
                    <div className="mb-4">
                        <div className="flex items-center justify-between mb-1"><div className="flex items-center gap-2"><span className="text-lg font-medium text-slate-400 line-through decoration-slate-400/50">{oldPrice.toLocaleString()} ₽</span><span className="bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded shadow-sm">-20%</span></div></div>
                        <div className="flex items-end justify-between"><p className="text-3xl font-black text-slate-900 leading-none tracking-tight">{price.toLocaleString()} ₽</p><div className="flex items-center gap-1 text-green-600 text-xs font-bold bg-green-50 px-2 py-1 rounded"><TrendingDown size={14} /><span>Выгода {savings.toLocaleString()} ₽</span></div></div>
                    </div>
                    <button onClick={handleOrder} className="w-full bg-slate-900 text-white font-bold py-3.5 px-6 rounded-xl shadow-lg flex justify-center gap-3 active:scale-[0.98]"><span>Оформить заявку</span></button>
                </div>
            </div>
