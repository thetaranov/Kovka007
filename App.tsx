...
import { Menu, X, FileText, Globe, TrendingDown, Send, Copy, Download, Mail } from 'lucide-react';
...
// ... BrowserOrderModal component code ... 
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const widthPenalty = Math.max(0, config.width - 4) * PRICING.baseTrussStructure.widthFactor;
    ...
    // 2. Столбы (Зависит от высоты и сечения)
    const maxSpan = 6.0;
    ...
    // 3. Кровля (С учетом реальной площади)
    let roofAreaMultiplier = 1.1; 
    if (config.roofType === RoofType.Gable) roofAreaMultiplier = 1.3;
    if (config.roofType === RoofType.Arched) roofAreaMultiplier = 1.4;
    if (config.roofType === RoofType.SemiArched) roofAreaMultiplier = 1.5;
    ...
    if (config.hasFoundation) {
        materialCost += floorArea * PRICING.extras.foundation;
    }
    ...
    // 7. ЖЕСТКОЕ ОГРАНИЧЕНИЕ МИНИМУМА (7000р/м2)
  ...
  const getOrderPayload = () => {
    ...
    return {
        ...
        area_floor: (config.width * config.length).toFixed(2),
        area_roof: (config.width * config.length * 1.2).toFixed(2),
