  ...
  return (
    <group>
      {geometry.elements.map((elem, idx) => {
        ...
        return (<BoxBeam key={idx} start={start} end={end} thickness={thickness} depth={depth} color={color} />);
      })}
    </group>
  );
};
    ...
    if (roofType === RoofType.Gable) {
       ...
       return (
        ...
        for (let i=0; i<segments; i++) {
           ...
           strips.push(
               <mesh key={i} position={position} rotation-z={angle}>
                   <boxGeometry args={[length, 0.01, totalL]} />
                   <meshStandardMaterial color={roofColor} transparent={isTrans} opacity={opacity} metalness={metalness} roughness={roughness} side={THREE.DoubleSide} />
               </mesh>
           );
        }
        ...
        Array.from({length: trussCount}).map((_, i) => {
          ...
          return (
            <group key={`calculated-truss-${i}`} position={[0, height + beamH, z]}>
              <CalculatedTruss geometry={calculation.geometry} sections={calculation.sections} color={frameColor} />
            </group>
          );
        })
      ) : (
