      ...
      <div className="max-w-7xl mx-auto p-4 lg:p-8">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 pb-6 border-b border-slate-200">
          ...
          <div className="flex gap-3 w-full lg:w-auto">
            <button
              onClick={onBack}
              className="flex-1 lg:flex-none px-6 py-3 border border-slate-300 rounded-xl hover:bg-slate-50 font-medium text-slate-700 flex items-center justify-center gap-2 transition-colors"
            >
              <ArrowLeft size={18} />
              <span>Назад</span>
            </button>
            <button
              ...
              className="flex-1 lg:flex-none px-6 py-3 bg-green-600 hover:bg-green-700 text-white rounded-xl font-bold shadow-lg shadow-green-200 flex items-center justify-center gap-2 transition-all active:scale-95"
            >
              <Download size={18} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between py-2 border-b border-slate-200/50">
                  <span className="text-slate-500">Габариты:</span>
                  <span className="font-bold font-mono">{config.width}x{config.length}x{config.height} м</span>
                </div>
                <div className="flex justify-between py-2 border-b border-slate-200/50">
                  <span className="text-slate-500">Высота фермы:</span>
                  <span className="font-bold font-mono">{calculation.geometry.height.toFixed(2)} м</span>
                </div>
                <div className="flex justify-between py-2 border-b border-slate-200/50">
                  <span className="text-slate-500">Шаг ферм:</span>
                  <span className="font-bold font-mono">{(config.length / Math.ceil(config.length/1.5)).toFixed(2)} м</span>
                </div>
                <div className="flex justify-between py-2">
                  <span className="text-slate-500">Вес каркаса:</span>
                  <span className="font-bold font-mono">{calculation.bom.totalWeight} кг</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
              ...
              <div className="space-y-3">
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                  <span className="text-xs text-slate-400 uppercase font-bold">Верхний пояс</span>
                  <div className="font-mono font-bold text-lg text-slate-700">{calculation.sections.topChord.name}</div>
                </div>
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                  <span className="text-xs text-slate-400 uppercase font-bold">Нижний пояс</span>
                  <div className="font-mono font-bold text-lg text-slate-700">{calculation.sections.bottomChord.name}</div>
                </div>
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                  <span className="text-xs text-slate-400 uppercase font-bold">Решетка</span>
                  <div className="font-mono font-bold text-lg text-slate-700">{calculation.sections.web.name}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            {/* Placeholder for "Baked" model - in real app could be a static image or a frozen Three.js canvas */}
            <div className="bg-slate-900 rounded-2xl aspect-video relative overflow-hidden flex items-center justify-center group">
               ...
               <div className="relative text-center p-6">
                  ...
                  <p className="text-slate-400 text-sm max-w-md mx-auto">Геометрия фермы оптимизирована под снеговые нагрузки вашего региона. Изменение параметров требует перерасчета.</p>
               </div>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
               <div className="p-4 border border-slate-200 rounded-xl bg-white hover:border-indigo-300 transition-colors cursor-pointer group">
                  <div className="font-bold text-slate-800 mb-1 group-hover:text-indigo-600">Чертежи КМД</div>
                  <p className="text-xs text-slate-500">Автоматически сгенерированные DXF файлы для лазерной резки и сборки.</p>
               </div>
               <div className="p-4 border border-slate-200 rounded-xl bg-white hover:border-indigo-300 transition-colors cursor-pointer group">
                  <div className="font-bold text-slate-800 mb-1 group-hover:text-indigo-600">Спецификация</div>
                  <p className="text-xs text-slate-500">Полный список профилей с длинами и углами реза.</p>
               </div>
            </div>
          </div>
        </div>
